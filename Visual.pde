public class Visual
{
  ...
  public void evadeMouse()
  {
    if (abs(loc.x - mouseX) < 50 && abs(loc.y - mouseY) < 50 )
    {
      PVector desVel = new PVector(loc.x - mouseX, loc.y - mouseY);
      desVel.mult(5);
      PVector steer = new PVector(desVel.x - vel.x, desVel.y - vel.y);
      vel.add(steer);
      move();
